...
int TestSubTreeLeafs(struct InputTree aTree,int source, int dest,struct DescTree *DTSpecies, struct DescTree *DTGene){
	...
	if(etape1==1 && etape2==1){
		...
		//printf("\n===========> nbSom = %d , basSource = %d , basDest = %d",nbSom,DTSpecies[basSource].nbSommet,DTSpecies[basDest].nbSommet);
		...
		for(y=1;y<=DTSpecies[basDest].nbSommet;y++){
			DTemp.Tableau[y] = DTSpecies[basDest].Tableau[y];
			//printf("%d ",DTSpecies[basDest].Tableau[y]);
		}
		...
		for(z=1;z<=DTSpecies[hautSource].nbSommet;z++){
			...
			//printf("%d ",DTSpecies[hautSource].Tableau[z]);
		}
...
/*
	for(i=1;i<= 2*size-2-kt;i++){
		...
		if ( fabs(ADD[i][size] - ADD[i][Sdest] - ADD[Sdest][size]) < 2*epsilon){
	//		printf(" -i=%d",i);
			for(j=1;j<=2*size-2-kt;j++){
				
				if( fabs(ADD[j][size] - ADD[j][Sdest] - ADD[Sdest][size]) > 2*epsilon ){
					
					ADD[i][j]  = ADD[j][i] = MIN(ADD[i][Sdest]+tailleTransfert+tailleBranche+ADD[Ssup][j],ADD[i][Sdest]+tailleTransfert+tailleBranche+ADD[Sinf][j]);
				}
			}
		}
		else{
			for(j=1;j<=2*size-2-kt;j++){
				...
				if( fabs(ADD[j][size] - ADD[j][Sdest] - ADD[Sdest][size]) < 2*epsilon ){
					...
					ADD[i][j] = ADD[j][i] = MIN(ADD[j][Sdest]+tailleTransfert+tailleBranche+ADD[Ssup][i],ADD[j][Sdest]+tailleTransfert+tailleBranche+ADD[Sinf][i]);
				}
			}
...
int TestCriterionAndUpdate(int *first,const char *CRITERION, struct CRITERIA aCrit, struct HGT * aHGT,int i,int j,int flag,int flag_bootstrap){
	...
	if (flag_bootstrap == 1){
		...
		//printf("\n==> rand_bootstrap = %d, best = %d",rand_bootstrap,best);
	}
	...
	//printf("\n>>>> DANS testCRiterionAndUpdate : RF = %d, BD =%lf",aCrit.RF,aCrit.BD);
...
int isAValidHGT(struct InputTree SpeciesTree,int i, int j){
	...
	//printf("\nhgt : i=%d , j=%d => %d-%d-%d-%d-%d",i,j,root,d1,d2,d3,d4);
...
//
//===================================================================================================================================
int readInputFile(FILE *in, const char *tmpFile/*, struct InputTree *speciesTree, struct InputTree *geneTree*/,char *fichier_erreur){
		...
		for(i=1;i<2*size-3-SpeciesTree.kt;i++)
			for(j=i+1;j<2*size-3-SpeciesTree.kt;j++){
				...
				if(isAValidHGT(SpeciesTree,i,j)==1 && i!=j ){
					...
					//printf("\ninitial = %d,initial3= %d",*initial,initial3);
					if(((aCritRef.RF-aCrit.RF) == 1) && (*initial==1) && (
					  ...
					  (SpeciesTree.ARETE[2*i-2] == SpeciesTree.ARETE[2*j-2])
						...
						if(TestCriterionAndUpdate(&first,param.criterion,aCrit,&aHGT[(*nbHgtFound)],i,j,0,0) == 1){		
							//printf("\n===========> flag3 = %d, initial3=%d",flag3,initial3);
							//printf("\tRF1=%d | RF2=%d | BD1=%2.1lf | BD2=%2.1lf",aHGT[(*nbHgtFound)].crit.RF,aCrit.RF,aHGT[(*nbHgtFound)].crit.BD,aCrit.BD);
							aHGT[(*nbHgtFound)].source_A = SpeciesTree.ARETE[2*i-1];
							aHGT[(*nbHgtFound)].source_B = SpeciesTree.ARETE[2*i-2];
							aHGT[(*nbHgtFound)].dest_A = SpeciesTree.ARETE[2*j-1];
...
int readParameters(struct Parameters * param, char **argv, int nargc){
	...
	//printf("\nhgt : reading parameters (%d)",nargc);
	
	for(i=1;i<nargc;i++){

		if(ExtraireDonnees(argv[i],champs,contenu)){

			//printf("\nhgt : [%s] -> champs = %s , contenu = %s",argv[i],champs,contenu);
			...
			//============ criterion ================
				...
				if(strcmp(contenu,"rf")!=0 && strcmp(contenu,"bd")!=0 && strcmp(contenu,"ls")!=0){
					printf("\nhgt : invalid criterion, accepted values : rf, bd and ls");
					exit(-1);
				}
			}//============ stepbystep ================
			else if(strcmp("stepbystep",champs) == 0){
				strcpy((*param).stepbystep,contenu);
				if(strcmp(contenu,"yes")!=0 && strcmp(contenu,"no")!=0){
					printf("\nhgt : invalid stepbystep value, accepted values : yes or no");
					exit(-1);
				}
			}
			//============= version ===============
			...
			//============ species root ===============
				...
				if(strcmp(contenu,"file")!=0 && strcmp(contenu,"midpoint")!=0){
					printf("\nhgt : invalid speciesroot, accepted values : file and midpoint");
				exit(-1);
				}
			}
			//============ gene root ===============
				...
				if(strcmp(contenu,"file")!=0 && strcmp(contenu,"midpoint")!=0 && strcmp(contenu,"bestrfbranch")!=0){
					printf("\nhgt : invalid generoot, accepted values : file, bestbipartition and midpoint");
					exit(-1);
				}
			}
			//======== load structure ==========
			...
			//============ nbHGT ===========
			else if(strcmp("randbootstrap",champs) == 0){
				(*param).rand_bootstrap = atoi(contenu);
			}
			//============ sort ===========
			else if(strcmp("sort",champs) == 0){
				sprintf((*param).sort,"%s",contenu);
				if(strcmp(contenu,"yes") != 0 && strcmp(contenu,"no") !=0){
					printf("\nhgt : incorrect value for [sort], accepted values : yes or no");
					exit(-1);
				}
			}
			//============ bootstrap ===========
				...
				if(strcmp(contenu,"yes") != 0 && strcmp(contenu,"no") !=0){
					printf("\nhgt : incorrect value for [bootstrap], accepted values : yes or no");
					exit(-1);
				}
			}
			//============ multigene ===========
			...
			//============ mode ===========
				...
				if(strcmp(contenu,"multicheck") != 0 && strcmp(contenu,"monocheck") !=0){
					printf("\nincorrect value for %s, accepted values : multicheck or monocheck",(*param).mode);
					exit(-1);
				}
			}
			//============ verbose ===========
				...
				if(strcmp(contenu,"yes") != 0 && strcmp(contenu,"no") !=0){
					printf("\nincorrect value for %s, accepted values : yes or no",(*param).verbose);
					exit(-1);
				}
			}
			else{
				printf("\nhgt : incorrect prameter : %s",champs);
				exit(-1);
			}
		}
		else{
			printf("\nhgt : invalid syntax for parameter : %s (needed -parameter=value)" , argv[i]);
			exit(-1);
		}
	}
	
	if(strcmp(input,"")==0){
		printf("\nerror hgt : there is no input file");
		exit(-1);
	}
