...
int trex(char*input,char*treeFile,char*statFile,long int **lgintARETE,double **dbLONGUEUR,int ecrireResultat);
...
void formatPhylip( char *input){
	...
	if(m_dataType == 2){
		...
		if(retour<2) { printf("Bad file format la!<br>"); printf("%d ",retour);exit(8);
		}
		fprintf(out,"%d %d\n",nbSpecies,taille);
	}
	if(m_dataType == 1){
	...
	if((p_optionTR == 1)||(p_optionTR == 2)){ /*inférence d'arbres ou de réticulogramme à partir de matrice*/
		if((m_dataType == 2)&&(p_boot == 2)){ /* sequences sans bootstrap*/
			...
			if(retour==0){
				/*printf("Est-ce la?-----------");
				printf ("retour: %d \n", retour);*/
				retour = trex(p_matrix,p_output,p_stat,&ARETE,&LONGUEUR,1);
			}
		}
		...
		if(m_dataType == 1){
			/*printf("Icit3-----------");*/
			retour = trex(p_input,p_output,p_stat,&ARETE,&LONGUEUR,1);
		}
	}
	else{
		/*printf("Icit4-----------");*/
...
int bootstrap_function(){
	...
	trex(p_matrix,"","",&ARETE,&LONGUEUR,0);     
	...
	/*ARETE    = (long int*)malloc((4*intNbreEspeces+3)*sizeof(long int));
	LONGUEUR = (double*)malloc((2*intNbreEspeces+3)*sizeof(double));*/
	...
	fclose(bs);
	trex(p_matrix,p_output,p_stat,&ARETE,&LONGUEUR,1);
...
int trex(char *inputFile,char *treeFile, char *statFile,long int **ARETE_,double **LONGUEUR_,int ecrireResultats){
	...
	if ((data=fopen(inputFile,"r"))==0) { printf("File %s was not found 2\n", p_input); exit(8); } 
	...
	for (i=1; i<=n; i++)
	{
		...
		k=0;
		while(k<10){
			chCaractere = getc(data);
			if(chCaractere == ' ')
					chCaractere = '_'; 
			Names[i-1][k] = chCaractere;
			k++;
		}
		Names[i-1][10] = '\0';
		j=9;
	...
	/* Tree reconstruction from comlete matrices and reticulogram reconstruction*/
		...
		if (method==5) /* options for the MW method*/
		{
			...
			{   
				FILE *weights;   
				if ((weights=fopen(mwFile,"r"))==0) { printf("*File %s was not found 1", mwFile); 
				exit(8); } 
				for (j=2; j<=n; j++)
				{
					for (i=1; i<=j-1; i++)
	...
	if(ecrireResultats){
		...
		if (TreeFile==NULL) {printf("\nFichier %s inexistant 1",treeFile); exit(8);}
		
	
		mod = fopen(p_modele,"r");
		if (mod==NULL) {printf("\nFichier %s inexistant 2",p_modele); exit(8);}
...
void BioNJ_main (double **DI, double **DA)
{
	...
	*b=0;
	...
	while (r > 3)                             /* until r=3                 */
	{
		...
		/*if (m_blnBarreProgress)
		{ 
			Pr=floor1((1000 * (n-r)) / n);
			m_ProgressDlg->m_Progress.SetPos(Pr);
			m_ProgressDlg->m_Progress.UpdateData(FALSE);
		}	*/
