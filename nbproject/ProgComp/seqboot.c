  ...
  while (!allread) {
    ...
    while (i <= spp) {
      ...
      if ((interleaved && j != basesnew) || ((!interleaved) && j != sites)){
        printf("\n<br>Sequences out of alignment at site %ld", j+1);
...
{ /* reads the input data */
  ...
  if (justwts && !permute)
    ...
    // Mazouzi
	  ...
	  //openfile(&outfile,OUTFILE,"output data file","w","G:\\Trex14vs21\\Debug\\TREX14.exe",outfilename);
...
int main_bootstrap(int argc, Char *argv[])
