...
{ /* input random number seed */
  ...
  // Récupération d'un nombre aléatoire, et qui doit être impair
  time_t t;
...
void initname(long i)
{
  ...
  //long j;
  long j = 0, k;
  char chCaratere = gettc(infile);
  
  //for (j = 0; j < nmlngth; j++) {
  while ((j < nmlngth) && (chCaratere != '\t') && (chCaratere != ' ')) {
  // Mazouzi - fin
	...
	nayme[i][j] = chCaratere;
	chCaratere = gettc(infile);
	// Mazouzi - fin
